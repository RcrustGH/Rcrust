...
if(is.na(as.numeric(chk))){return(FALSE)}else{
return(TRUE)
}
}
}
...
if(class(chk)=="try-error"){val<-0}else{
...
check_tuple<-function(tuple=""){
  if(tuple==""){return(list("Valid tuple",""))}
  split_tuple<-unlist(strsplit(gsub("\\{","",gsub("\\}","",tuple)),split=",|;"))
  if(!length(split_tuple)==2){return(list("Error: Tuple is invalid",NULL))}
  for(k in 1:length(split_tuple)){
    if(is.na(suppressWarnings(as.numeric(split_tuple[k])))){return(list("Error: Tuple argument is non-numeric",NULL))}
    if(as.numeric(split_tuple[k])<1){return(list("Error: Tuple argument is less than 1",NULL))}
    if(!as.numeric(split_tuple[k])%%1==0){return(list("Error: Tuple argument is not a whole number",NULL))}
  }
...
phase_abundance<-function(crust,axis,path=1,p_a=1,p_b=p_a,path_label="Point",input_pt=NULL){
  ...
  #Abundance rows
  switch(axis,
          x={pnt_y<-"path";pnt_x<-"p_i"},
          y={pnt_y<-"p_i";pnt_x<-"path"}
          )
    all_phases<-NULL
    for(p_i in p_a:p_b){
    ...
    switch(path_label,"Pressure(kbar)"=slct<-1,"Temperature(C)"=slct<-2)
    if(!is.null(input_pt)){
      col_nms<-NULL
      for(p_i in p_a:p_b){
...
grab[i]<-substr(grab[i],1,nchar(grab[i])-3)
		...
		if(collapsed==""){collapsed<-"No Phases"}
...
PAM_names<-as.character(compilation_names[[2]])}else{
...
error_handling<-function(working_file,projects_directory){
  if(projects_directory==""){return("Error: no projects directory specified")}
  if(!dir.exists(projects_directory)){return("Error: projects directory does not exist")}
  if(working_file==""){return("Error: no working file specified")}
...
shinyServer(function(input, output,session) {
  ...
  if(!exists("major_elements")){major_elements<-""}
  #if(!is.null(major_elements)){if(major_elements[1]==""){major_elements<-NULL}}
  if(!exists("trace_elements")){trace_elements<-""}
  #if(!is.null(trace_elements)){if(trace_elements[1]==""){trace_elements<-NULL}}
  if(!exists("all_elements")){all_elements<-c(major_elements,trace_elements)
  ...
  on_save <- reactive({
	...
	if(!exists("from_copy")){from_copy<<-FALSE}
	if(from_copy){
    ...
    if(reactive_message$data=="error handling passed"){
      ...
      if(!input$x_n==""){if(is.na(suppressWarnings(as.numeric(input$x_n)))){return("Error: X must be numeric")}
      if(as.numeric(input$x_n)<1){return("Error: X must be greater than 1")}
      if(!as.numeric(input$x_n)%%1==0){return("Error: X must be a whole number")}}
      if(!input$y_n==""){if(is.na(suppressWarnings(as.numeric(input$y_n)))){return("Error: Y must be numeric")}
      if(as.numeric(input$y_n)<1){return("Error: Y must be greater than 1")}
      if(!as.numeric(input$y_n)%%1==0){return("Error: Y must be a whole number")}}
        ...
        if(!input$n_pt_def==""){
          #Error validation
        if(is.na(suppressWarnings(as.numeric(input$n_pt_def)))){return("Error: Number of PT definitions must be numeric")}
        if(as.numeric(input$n_pt_def)<1){return("Error: Number of PT definitions must be greater than 0")}
        if(!as.numeric(input$n_pt_def)%%1==0){return("Error: Number of PT definitions must be a whole number")}
        list_pt<-NULL
      ...
      if(input$n_comp_trans=="load"){
        ...
        if(!input$n_comp_trans==""){
          #Error validation
          if(is.na(suppressWarnings(as.numeric(input$n_comp_trans)))){return("Error: Number of Component transformations must be numeric")}
          if(as.numeric(input$n_comp_trans)<1){return("Error: Number of Component transformations must be greater than 0")}
          if(!as.numeric(input$n_comp_trans)%%1==0){return("Error: Number of Component transformations must be a whole number")}
          list_trans<-NULL
      ...
      if(input$n_bulk_def=="load"){
        ...
        if(!input$n_bulk_def==""){
          #Error validation
          if(is.na(suppressWarnings(as.numeric(input$n_bulk_def)))){return("Error: Number of Bulk definitions must be numeric")}
          if(as.numeric(input$n_bulk_def)<1){return("Error: Number of Bulk definitions must be greater than 0")}
          if(!as.numeric(input$n_bulk_def)%%1==0){return("Error: Number of Bulk definitions must be a whole number")}
          list_bulk<-NULL
      ...
      if(input$ph_add){
        if(input$n_ph_add_def=="load"){
          ...
          if(!input$n_ph_add_def==""){
            #Error validation
            if(is.na(suppressWarnings(as.numeric(input$n_ph_add_def)))){return("Error: Number of Phase Addition definitions must be numeric")}
            if(as.numeric(input$n_ph_add_def)<1){return("Error: Number of Phase Addition definitions must be greater than 0")}
            if(!as.numeric(input$n_ph_add_def)%%1==0){return("Error: Number of Phase Addition definitions must be a whole number")}
            list_ph_add<-NULL
      ...
      if(input$ph_extr){
        if(input$n_ph_extr_def=="load"){
          ...
          if(!input$n_ph_extr_def==""){
            #Error validation
            if(is.na(suppressWarnings(as.numeric(input$n_ph_extr_def)))){return("Error: Number of Phase Extraction definitions must be numeric")}
            if(as.numeric(input$n_ph_extr_def)<1){return("Error: Number of Phase Extraction definitions must be greater than 0")}
            if(!as.numeric(input$n_ph_extr_def)%%1==0){return("Error: Number of Phase Extraction definitions must be a whole number")}
            list_ph_extr<-NULL
		...
		if(input$cp_components=="load"){
			if(!all(cp_components_r$data=="")){
				...
				for(i in 1:length(cp_components_r$data)){
					...
					}else{cp_phases_combine<-c(cp_phases_combine,paste0("cp_phases_",names(cp_components_r$data[i]),"<-c(",paste0(cp_phases_defs,collapse=","),")"))}
			...
			}else{cp_packet_definitions<-"cp_phases_<-c(\"\")"
				list_cp_isolate<-""
			}
		}else{
			if(!isTRUE(input$cp_components=="")){
				...
				for(i in 1:as.numeric(length(packet_components))){
					...
					}else{cp_phases_combine<-c(cp_phases_combine,paste0("cp_phases_",packet_components[i],"<-c(",paste0(cp_phases_defs,collapse=","),")"))}
				}
				#combined into a string to store in text file
				cp_packet_definitions<-paste0(cp_phases_combine,collapse="")
				list_cp_isolate<-paste0(list_cp_isolate,collapse=",")
	  ...
	  w_component_packet<-paste0("###############\n#\n#   Component packet\n#\n###############\n",
        ...
        if(input$solution_models_file=="load"){
          if(!exists("solution_models_file")){solution_models_file<-""}
        }else{
	...
	if(input$meemum_path==""){return("Error: No meemum path defined")}
	if(!file.exists(paste0(gsub("/code","/data",getwd()),"/",input$meemum_path))){return(paste0("Error: Meemum not found at ",paste0(gsub("/code","/data",getwd()),"/",input$meemum_path)))}
  ...
  on_load <- reactive({
    ...
    if(reactive_message$data=="error handling passed"){
      ...
      #custom loads
      #bulk_def
      if(exists("bulk_def")){if(bulk_def=="input"){bulk_def_file<-FALSE}else{bulk_def_file<-TRUE}}else{bulk_def_file<-FALSE}
      #get addition phases
      if(is.null(ph_add_definitions_r$data[[1]][1])){add_phases<-""}else{
        add_phases<-paste(setdiff(names(ph_add_definitions_r$data[[1]]),"condition"),collapse=",")
      }
	  #get extract phases
      if(is.null(ph_extr_definitions_r$data[[1]][1])){extr_phases<-""}else{
      ...
      for(i in 1:length(load_variables)){
        ...
        if(load_variables[i]=="def_count"){
          ...
          if(exists(def_name)){
            if(is.null(eval(parse(text=def_name))[[1]][1])){val_out<-""}
            else{if(eval(parse(text=def_name))[[1]][1]==""){val_out<-""}
              else{val_out<-length(eval(parse(text=def_name)))}}}else{val_out<-""}
  ...
  projects_directory<-input$projects_directory
    ...
    reactive_message$data <-error_handling(working_file,projects_directory)
      if(reactive_message$data=="error handling passed"){
        ...
        if(exists("input_bulk")){store_r$input_bulk_r<-input_bulk}
        if(exists("major_elements")){store_r$major_elements_r<-major_elements}
		if(exists("trace_elements")){store_r$trace_elements_r<-trace_elements}
		if(exists("all_elements")){store_r$all_elements_r<-all_elements}
    }else{
  ...
  observeEvent(input$run, {
    ...
    if(reactive_message$data=="error handling passed"){
		...
		for(i in rev(chk_variables)){if(any(eval(parse(text=i))=="")){var_missing<-i}}
	  ...
	  #Save copy to directory
		if(FALSE){copy_directory<<-"H:/Rcrust/Projects"
		from_copy<<-TRUE
	    reactive_message$data <- paste0(on_save())
	}
	  #End of Calculation
if(!exists("end_of_calc")){end_of_calc<-"Return to Interface"}
	  #Email report
	  if(FALSE){
	  library(gmailr)
	  mime() %>%
...
if(end_of_calc=="Logout"){system('shutdown -l')}
if(end_of_calc=="Shutdown"){system('shutdown -s')}
      #Refresh reactive outputs if they exist
      if(exists("crust")){store_r$crust_r<-crust}
      if(exists("input_pt")){store_r$input_pt_r<-input_pt}
      if(exists("input_bulk")){store_r$input_bulk_r<-input_bulk}
  ...
  if (is.null(reactive_message$data)) return()
  ...
  # Dyanmically use number of PT definitions to create the correct number of From,To,P,T inputs
    ...
    if(input$n_pt_def=="load"){
      ...
      validate(if(is.na(suppressWarnings(as.numeric(def_num)))){"Error: Number of PT definitions must be numeric"}else{NULL})
      validate(if(as.numeric(def_num)<1){"Error: Number of PT definitions must be greater than 0"}else{NULL})
      validate(if(!as.numeric(def_num)%%1==0){"Error: Number of PT definitions must be a whole number"}else{NULL})
      fixedRow(
        lapply(1:(as.numeric(def_num)*4), function(i) {
          a<-1; ii<-i
          while(ii>4){ii<-ii-4;a<-a+1}
          if(ii==1){chk_From<-try(unlist(strsplit(names(pt_definitions_r$data)[a],split="_"))[1],silent=TRUE)
                    if(class(chk_From)=="try-error"){chk_From<-NULL}
                    column(2,textInput(paste0('pt_from_',a),'From',value=chk_From))}else{
          if(ii==2){chk_To<-try(unlist(strsplit(names(pt_definitions_r$data)[a],split="_"))[2],silent=TRUE)
                    if(class(chk_To)=="try-error"){chk_To<-NULL}
                    column(2,textInput(paste0('pt_to_',a),'To',value=chk_To))}else{
          if(ii==3){chk_pressure<-try(pt_definitions_r$data[[a]][1],silent=TRUE)
                    if(class(chk_pressure)=="try-error"){chk_pressure<-NULL}
                    column(4,textInput(paste0('pressure_',a),'Pressure (kbar)',value=chk_pressure))}else{
          if(ii==4){chk_temperature<-try(pt_definitions_r$data[[a]][2],silent=TRUE)
                    if(class(chk_temperature)=="try-error"){chk_temperature<-NULL}
  ...
  if(line_no==1){lines_all<-paste("pt_definitions<-list(",line_i,sep="")}else{
  ...
  reactive_message$data <- paste0(on_load())
    ...
    # Dyanmically use number of component transformations to create the correct number of inputs
    output$trans <- renderUI({
      ...
      if(!(is.null(def_num)|def_num==""|def_num==0)){
        validate(if(is.na(suppressWarnings(as.numeric(def_num)))){"Error: Number of Component transformations must be numeric"}else{NULL})
        validate(if(as.numeric(def_num)<1){"Error: Number of Component transformations must be greater than 0"}else{NULL})
        validate(if(!as.numeric(def_num)%%1==0){"Error: Number of Components transformations must be a whole number"}else{NULL})
        fixedRow(
          lapply(1:(as.numeric(def_num)*3), function(i) {
            a<-1; ii<-i
            while(ii>3){ii<-ii-3;a<-a+1}
            if(ii==1){chk_old<-try(unlist(strsplit(names(comp_transformations_r$data)[a],split="_"))[1],silent=TRUE)
            if(class(chk_old)=="try-error"){chk_old<-NULL}
            column(2,textInput(paste0('old_comp_',a),'Replace component',value=chk_old))}else{
            ...
            if(ii==2){chk_new<-try(unlist(strsplit(names(comp_transformations_r$data)[a],split="_"))[2],silent=TRUE)
                      if(class(chk_new)=="try-error"){chk_new<-NULL}
                      column(2,textInput(paste0('new_comp_',a),'New component',value=chk_new))}else{
            if(ii==3){
                  comp_label<-paste(available_components_r$current,collapse = ",")
                  chk_comp<-try(comp_transformations_r$data[[a]],silent=TRUE)
                  if(class(chk_comp)=="try-error"){chk_comp<-NULL}
                  column(8,textInput(paste0('comp_',a),comp_label,value=paste(chk_comp,collapse=",")))}else{
                  }}}})
        )
      }
    })
    ...
    output$bulk <- renderUI({
      ...
      if(!(is.null(def_num)|def_num==""|def_num==0)){
        validate(if(is.na(suppressWarnings(as.numeric(def_num)))){"Error: Number of Bulk definitions must be numeric"}else{NULL})
        validate(if(as.numeric(def_num)<1){"Error: Number of Bulk definitions must be greater than 0"}else{NULL})
        validate(if(!as.numeric(def_num)%%1==0){"Error: Number of Bulk definitions must be a whole number"}else{NULL})
        fixedRow(
          lapply(1:(as.numeric(def_num)*3), function(i) {
            a<-1; ii<-i
            while(ii>3){ii<-ii-3;a<-a+1}
            if(ii==1){chk_From<-try(unlist(strsplit(names(bulk_definitions_r$data)[a],split="_"))[1],silent=TRUE)
            if(class(chk_From)=="try-error"){chk_From<-NULL}
            column(2,textInput(paste0('bulk_from_',a),'From',value=chk_From))}else{
              if(ii==2){chk_To<-try(unlist(strsplit(names(bulk_definitions_r$data)[a],split="_"))[2],silent=TRUE)
              if(class(chk_To)=="try-error"){chk_To<-NULL}
              column(2,textInput(paste0('bulk_to_',a),'To',value=chk_To))}else{
                if(ii==3){
                  if(is.null(input$major_elements)){
                ...
                if(class(chk_bulk)=="try-error"){chk_bulk<-NULL}
                column(8,textInput(paste0('bulk_',a),bulk_label,value=paste(chk_bulk,collapse=",")))}else{
    }}}})
        )
      }
    })
 #Import bulk definitions button
  ...
  if(line_no==1){lines_all<-paste("bulk_definitions<-c(list(",line_i,sep="")}else{
  lines_all<-paste(lines_all,line_i,sep=",")
  }
  if(line_no==nrow(data_in)){
  lines_all<-paste(lines_all,"))",sep="")
  ...
  reactive_message$data <- paste0(on_load())
    ...
    output$ph_add <- renderUI({
      ...
      if(!(is.null(def_num)|def_num==""|def_num==0)){
        validate(if(is.na(suppressWarnings(as.numeric(def_num)))){"Error: Number of Phase Addition definitions must be numeric"}else{NULL})
        validate(if(as.numeric(def_num)<1){"Error: Number of Phase Addition definitions must be greater than 0"}else{NULL})
        validate(if(!as.numeric(def_num)%%1==0){"Error: Number of Phase Addition definitions must be a whole number"}else{NULL})
        fixedRow(
          lapply(1:(as.numeric(def_num)*5), function(i) {
            a<-1; ii<-i
            while(ii>5){ii<-ii-5;a<-a+1}
            if(ii==1){chk_From<-try(unlist(strsplit(names(ph_add_definitions_r$data)[a],split="_"))[1],silent=TRUE)
            if(class(chk_From)=="try-error"){chk_From<-NULL}
            column(2,textInput(paste0('ph_add_from_',a),'From',value=chk_From))}else{
              if(ii==2){chk_To<-try(unlist(strsplit(names(ph_add_definitions_r$data)[a],split="_"))[2],silent=TRUE)
              if(class(chk_To)=="try-error"){chk_To<-NULL}
              column(2,textInput(paste0('ph_add_to_',a),'To',value=chk_To))}else{
				if(ii==3){chk_Condition<-try(ph_add_definitions_r$data[[a]][["condition"]],silent=TRUE)
                if(class(chk_Condition)=="try-error"){chk_Condition<-NULL}
                column(3,textInput(paste0('ph_add_con_',a),'Condition',value=chk_Condition))}else{
				if(ii==4){
				chk_Phases<-try(paste(names(ph_add_definitions_r$data[[a]][-1]),collapse=","),silent=TRUE)
                if(class(chk_Phases)=="try-error"){chk_Phases<-NULL}
  ...
  if(line_no==1){lines_all<-paste("ph_add_definitions<-c(list(",line_i,sep="")}else{
  lines_all<-paste(lines_all,line_i,sep=",")
  }
  if(line_no==nrow(data_in)){
  lines_all<-paste(lines_all,"))",sep="")
  ...
  for(line_no in 1:nrow(data_in)){
  line_i<-paste("\"{",data_in[line_no,1],";",data_in[line_no,2],"}_{",data_in[line_no,3],";",data_in[line_no,4],"}\"=c(condition=\"",data_in[line_no,5],"\",",data_in[line_no,6],")",sep="")
  if(line_no==1){lines_all<-paste("ph_extr_definitions<-c(list(",line_i,sep="")}else{
  lines_all<-paste(lines_all,line_i,sep=",")
  }
  if(line_no==nrow(data_in)){
  lines_all<-paste(lines_all,"))",sep="")
  ...
  updateTextInput(session,input$file_ph_extr,value="")
    ...
    output$ph_extr <- renderUI({
      ...
      if(!(is.null(def_num)|def_num==""|def_num==0)){
        validate(if(is.na(suppressWarnings(as.numeric(def_num)))){"Error: Number of Phase Extraction definitions must be numeric"}else{NULL})
        validate(if(as.numeric(def_num)<1){"Error: Number of Phase Extraction definitions must be greater than 0"}else{NULL})
        validate(if(!as.numeric(def_num)%%1==0){"Error: Number of Phase Extraction definitions must be a whole number"}else{NULL})
		fixedRow(
          lapply(1:(as.numeric(def_num)*5), function(i) {
            a<-1; ii<-i
			while(ii>5){ii<-ii-5;a<-a+1}
            if(ii==1){chk_From<-try(unlist(strsplit(names(ph_extr_definitions_r$data)[a],split="_"))[1],silent=TRUE)
            if(class(chk_From)=="try-error"){chk_From<-NULL}
            column(2,textInput(paste0('ph_extr_from_',a),'From',value=chk_From))}else{
              if(ii==2){chk_To<-try(unlist(strsplit(names(ph_extr_definitions_r$data)[a],split="_"))[2],silent=TRUE)
              if(class(chk_To)=="try-error"){chk_To<-NULL}
              column(2,textInput(paste0('ph_extr_to_',a),'To',value=chk_To))}else{
                if(ii==3){chk_Condition<-try(ph_extr_definitions_r$data[[a]][["condition"]],silent=TRUE)
                if(class(chk_Condition)=="try-error"){chk_Condition<-NULL}
                column(3,textInput(paste0('ph_extr_con_',a),'Condition',value=chk_Condition))}else{
				if(ii==4){
				...
				if(i>=2){chk_Phases<-paste0(chk_Phases,",")}
...
#remove phases<-unlist(strsplit(chk_Phases,split=\",\"))
    ...
    output$solution_models <- renderUI({
      ...
      if(!(is.null(input$solution_models_file)|input$solution_models_file=="")){
		if(!input$solution_models_file=="load"){
		validate(if(!dir.exists(gsub("/code","/data",getwd()))){return("Error: data directory does not exist")}else{NULL})
        validate(if(!file.exists(paste0(gsub("/code","/data",getwd()),"/",input$solution_models_file))){return("Error: Solution model file does not exist")}else{NULL})
    ...
    output$cp_packet_ui <- renderUI({
        ...
        fixedRow(
          lapply(1:(as.numeric(length(break_on_comma(input$cp_components)))*5), function(i) {
            a<-1; ii<-i 
            while(ii>5){ii<-ii-5;a<-a+1}
				if(ii==1){chk_cp_isolate<-try(cp_components_r$data[[a]],silent=TRUE)
					if(class(chk_cp_isolate)=="try-error"){chk_cp_isolate<-NULL}
					column(4,textInput(paste0('cp_isolate_',a),paste0('Proportion of component to isolate in ',break_on_comma(input$cp_components)[a]),value=chk_cp_isolate))}else{
				if(ii==2){chk_cp_phases<-try(paste0(names(eval(parse(text=paste0('cp_phases_',names(cp_components_r$data[a]))))),collapse=","),silent=TRUE)
					if(class(chk_cp_phases)=="try-error"){chk_cp_phases<-NULL}
	...
	if(!dir.exists(paste0(input$projects_directory,"/Compile"))){dir.create(paste0(input$projects_directory,"/Compile"))}
	compile_names<-NULL
	for(i in mergers){
    ...
    observeEvent(input$save_data, {
      switch(input$output_type,
             ...
             "Grid"=
			if(TRUE){reactive_message$data <-"Saving"
     ...
     grid_data_r <- reactive({
		...
		if(!is.na(grid_in[[2]][i])){
			if(length(unlist(strsplit(val,split="-")))>1){
				if(grid_in[[2]][i]>=unlist(strsplit(val,split="-"))[1]&grid_in[[2]][i]<=unlist(strsplit(val,split="-"))[2]){grid_in[[2]][i]<-NA}
			}else{
			if(grid_in[[2]][i]==val){grid_in[[2]][i]<-NA}}}}
		}
		}
		...
		if(any(input$reflection=="Horizontal")){grid_in[[2]]<-flip_y(grid_in[[2]])}
		if(any(input$reflection=="Vertical")){grid_in[[2]]<-flip_x(grid_in[[2]])}
	...
	if(any(input$Grid_axes=="bottom")){bottom<-create_Axes(input$Grid_bottom_axis,input$Grid_bottom_axis_grid_phase,input$Grid_bottom_axis_increments,"x",crust_out(),input_pt)}else{bottom<-NULL}
	if(any(input$Grid_axes=="left")){left<-create_Axes(input$Grid_left_axis,input$Grid_left_axis_grid_phase,input$Grid_left_axis_increments,"y",crust_out(),input_pt)}else{left<-NULL}
	if(any(input$Grid_axes=="top")){top<-create_Axes(input$Grid_top_axis,input$Grid_top_axis_grid_phase,input$Grid_top_axis_increments,"x",crust_out(),input_pt,y_n)}else{top<-NULL}
	if(any(input$Grid_axes=="right")){right<-create_Axes(input$Grid_right_axis,input$Grid_right_axis_grid_phase,input$Grid_right_axis_increments,"y",crust_out(),input_pt,x_n)}else{right<-NULL}
	...
	if(axes_increments=="Increments"){axes_increments<-11}else{axes_increments<-as.numeric(axes_increments)}
	if(axes_direction=="y"){
	i_n<-length(crust)/(axes_increments-1)*(0:(axes_increments-1))+1
	}else{i_n<-length(crust[[1]])/(axes_increments-1)*(0:(axes_increments-1))+1}
			...
			for(i in i_n[-length(i_n)]){
				if(axes_direction=="y"){
					...
					if(axes_variable=="Temperature (C)"){
						...
						if(class(chk)=="try-error"){
							axis_values<-c(axis_values,0)}else{
	...
	return(list(axis_title,signif(axis_values,digits=4)))
		...
		right<-grid_data_r()[[6]]
		action<-"View"
		if(!is.null(reactive_message$data)){if(reactive_message$data=="Saving"){action<-"Save"}}
		if(action=="Save"){
		...
		plot.axes={
		if(!is.null(bottom)){axis(1,(0:(length(bottom[[2]])-1))/(length(bottom[[2]])-1),bottom[[2]])}else{NULL};
		if(!is.null(left)){axis(2,(0:(length(left[[2]])-1))/(length(left[[2]])-1),left[[2]])}else{NULL};
		if(!is.null(top)){axis(3,(0:(length(top[[2]])-1))/(length(top[[2]])-1),top[[2]])}else{NULL};
		if(!is.null(right)){axis(4,(0:(length(right[[2]])-1))/(length(right[[2]])-1),right[[2]])}else{NULL}},
		...
		if(!is.null(bottom)){mtext(bottom[[1]],1,line=3)}
		if(!is.null(left)){mtext(left[[1]],2,line=3)}
		if(!is.null(top)){mtext(top[[1]],3,line=3)}
		if(!is.null(right)){mtext(right[[1]],4,line=3)}
		if(action=="Save"){
...
draw_abundance_r<-reactive({
		...
		phase_abundance_data<-apply(phase_abundance_data, 2, function(x){x*100/sum(x,na.rm=T)})
		#Set colour pallette
	...
	if(input$compile_PAM){write.table(compiled_legend,outfile_compile_legend_path,sep="\t",quote=F,row.names = TRUE,col.names = FALSE)}
  }
  if(input$file_type==".csv"){
    ...
    if(class(write_test)=="try-error"){
      ...
      return(paste0("Error could not save phase assemblage map legend: ",outfile_legend_path,", file may be open in another program, please close all programs that may be accessing the file then try again\n"))
		...
		if(input$compile_PAM){write_test<-try(write.csv(compiled_legend,outfile_compile_legend_path,row.names = TRUE,col.names = FALSE),silent=TRUE)
    if(class(write_test)=="try-error"){
	...
	increment_choice<-eval(parse(text=paste("input$PAM_",input$PAM_axes[j],"_axis_increments",sep="")))
		if(increment_choice=="Increments"){increment_choice<-11}else{increment_choice<-as.numeric(increment_choice)}
	if(input$PAM_axes[j]=="left"|input$PAM_axes[j]=="right"){
	i_n<-length(crust)/(increment_choice-1)*(0:(increment_choice-1))+1
	}else{i_n<-length(crust[[1]])/(increment_choice-1)*(0:(increment_choice-1))+1}
	...
	side_no<-switch(input$PAM_axes[j],"bottom"=1,"left"=2,"top"=3,"right"=4)
...
#Add contour
	...
	if(class(chk)=="try-error"){ grid_out_mat[y_i,x_i]<-0}else{
	grid_out_mat[y_i,x_i]<-chk
				...
				if(input$PAM_contour_increments=="Default Increments"){raster::contour(x,add=TRUE,col="red")}else{
				if(input$PAM_contour_increments=="In/Out"){raster::contour(x,add=TRUE,levels=0.0000000000000000001,col="red")}else{
...
return(paste0("Phase Assemblage Map saved to ",input$projects_directory,"/",input$working_file,"/Outputs/\n"))
    ...
    output$output_header<-renderText(
      switch(input$output_type,
             "Data File"=if(!is.null(store_r$crust_r)){"Compilation data file"},
             "Grid"=if(!is.null(grid_data_r())){paste0(grid_data_r()[[1]]," on (X,Y) grid")},
             "Phase Abundance Along Path"=if(!is.null(phase_abundance_r())){phase_abundance_r()[[1]]},
			 "PAM"=if(!is.null(store_r$crust_r)){"Phase Assemblage Map"}
    ))
    ...
    output$table <- renderTable(
      switch(input$output_type,
              ...
              "Grid"=if(!is.null(grid_data_r())){grid_data_r()[[2]]},
    ...
    output$output_selection <- renderUI({
      ...
      conditionalPanel("input.output_type == 'Data File'",
      ...
      conditionalPanel("input.output_type == 'Grid'",
		selectInput('Grid_variable', 'Variable', c("Pressure","Temperature",all_columns,"A/CNK","Custom"), selected="Pressure",selectize=TRUE),
		conditionalPanel(condition= "input.Grid_variable.indexOf('Custom') != -1",
			textInput('Custom_selection',"Custom Selection")
			),
		conditionalPanel("['Pressure','Temperature','Custom'].indexOf(input.Grid_variable) == -1",
        ...
        selectInput('Grid_axes', 'Labelled axes', c("bottom","left","top","right"), selected=c("bottom","left"),selectize=TRUE,multiple=TRUE),
					   conditionalPanel(condition= "input.Grid_axes.indexOf('bottom') != -1",
						...
						conditionalPanel("['x_i','Pressure','Temperature'].indexOf(input.Grid_bottom_axis) == -1",
						...
						conditionalPanel(condition= "input.Grid_axes.indexOf('left') != -1",
						...
						conditionalPanel("['y_i','Pressure','Temperature'].indexOf(input.Grid_left_axis) == -1",
						...
						selectizeInput('Grid_left_axis_increments', NULL, c("Increments",3:y_n),"Increments")
						  ...
						  conditionalPanel(condition= "input.Grid_axes.indexOf('top') != -1",
						...
						conditionalPanel("['x_i','Pressure','Temperature'].indexOf(input.Grid_top_axis) == -1",
						...
						selectizeInput('Grid_top_axis_increments', NULL, c("Increments",3:x_n),"Increments")
						  ...
						  conditionalPanel(condition= "input.Grid_axes.indexOf('right') != -1",
						...
						conditionalPanel("['y_i','Pressure','Temperature'].indexOf(input.Grid_right_axis) == -1",
	  ...
	  conditionalPanel("input.Grid_colours == 'RColorBrewer'",
      ...
      conditionalPanel("input.output_type == 'Phase Abundance Along Path'",
                       selectInput('axis', 'Axis', c("x","y"), selected="x",selectize=TRUE),
                       conditionalPanel("input.axis == 'x'",
	  ...
	  conditionalPanel("input.output_type == 'PAM'",
					   ...
					   conditionalPanel(condition= "input.PAM_axes.indexOf('bottom') != -1",
						selectInput('PAM_bottom_axis', 'Bottom Axis', c("x_i","Pressure","Temperature",colnames(crust[[1]][[1]])), selected="x_i",selectize=TRUE),
						conditionalPanel("['x_i','Pressure','Temperature'].indexOf(input.PAM_bottom_axis) == -1",
						...
						conditionalPanel(condition= "input.PAM_axes.indexOf('left') != -1",
						selectInput('PAM_left_axis', 'Left Axis', c("y_i","Pressure","Temperature",colnames(crust[[1]][[1]])), selected="y_i",selectize=TRUE),
						conditionalPanel("['y_i','Pressure','Temperature'].indexOf(input.PAM_left_axis) == -1",
						...
						selectizeInput('PAM_left_axis_increments', NULL, c("Increments",3:y_n),"Increments")
						  ...
						  conditionalPanel(condition= "input.PAM_axes.indexOf('top') != -1",
						selectInput('PAM_top_axis', 'Top Axis', c("x_i","Pressure","Temperature",colnames(crust[[1]][[1]])), selected="x_i",selectize=TRUE),
						conditionalPanel("['x_i','Pressure','Temperature'].indexOf(input.PAM_top_axis) == -1",
						...
						selectizeInput('PAM_top_axis_increments', NULL, c("Increments",3:x_n),"Increments")
						  ...
						  conditionalPanel(condition= "input.PAM_axes.indexOf('right') != -1",
						selectInput('PAM_right_axis', 'Right Axis', c("y_i","Pressure","Temperature",colnames(crust[[1]][[1]])), selected="y_i",selectize=TRUE),
						conditionalPanel("['y_i','Pressure','Temperature'].indexOf(input.PAM_right_axis) == -1",
						...
						conditionalPanel("['None','Pressure','Temperature'].indexOf(input.PAM_contour) == -1",
	  ...
	  #actionButton("send_gcdkit","Send To GCDkit")
		...
		observe({
		if(!exists("first_load")){first_load<<-TRUE}
		if(first_load){
		projects_directory<-input$projects_directory
	    working_file<-input$working_file
		 #error handling
