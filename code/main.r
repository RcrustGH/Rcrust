...
sec_to_clock <- function(total_sec) {
  ...
  if(mn<10){mn_0<-"0"}else{mn_0<-NULL}
  sec<-total_sec-hr*3600-mn*60
  if(sec<10){sec_0<-"0"}else{sec_0<-NULL}
          ...
          for (k in 1:length(c0)) {
            ...
            if (left_brac[1]>0) {
              ...
              if (k==1&ph_extr&cumulate_on_rs) {
                ...
                if(!length(parse_cumul_extract)==0){cumul_extract[[y_i]][[x_i]]<-parse_cumul_extract}
              }
              ...
              if (k==1&ph_add&cumulate_on_rs) {
                ...
                if(!length(parse_cumul_addition)==0){cumul_add[[y_i]][[x_i]]<-parse_cumul_addition}
              }
              #substitute in value
        ...
        if (calc_mode=="normal") {
          ...
          for (i in 1:length(calc_order[[j]])) {
            ...
            if (silent_calc) {
              ...
              crust[[y_i]][[x_i]]<-run.Rcrust(comps=comps,c0=input_bulk[[y_i]][[x_i]],
                    press=input_pt[[y_i]][[x_i]][1],
                    temp=input_pt[[y_i]][[x_i]][2],
