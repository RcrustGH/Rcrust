...
for (y_i in 1:y_n) {
  for (x_i in 1:x_n) {
    if (is.null(dependencies[[y_i]][[x_i]])) {
      cat("Error: No dependence defined for x_i =", x_i, " y_i =", y_i, " \n")
      stop()
    }
  }
}
if (!length(unlist(calc_order)) / 2 == x_n * y_n) {
  cat("Error: Calculation order could not be determined\n")
  stop()
}
