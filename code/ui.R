...
if(!exists("working_file")){working_file<-""}
shinyUI(fixedPage(
...
# Logo for Rcrust
  ...
  fixedRow(textInputRow("projects_directory","Projects Directory",value=paste0(substring(getwd(),1,nchar(getwd())-4),"Projects")),
           ...
           actionButton("save","Save"),actionButton("load","Load"),actionButton("run","Run"),actionButton("clear","Clear"),actionButton("console","Console"),align="right"),
...
tabPanel("Input Parameters",
  #Size Panel
  wellPanel("Size",actionButton("size_panel_minimiser", label = "..."),
  conditionalPanel("(input.size_panel_minimiser)%2 == 0",
  fixedRow(
  ...
  #PT Panel
  wellPanel("Pressure and Temperature",actionButton("pt_panel_minimiser", label = "..."),actionButton("pt_import_panel_minimiser", label = "<-"),
  conditionalPanel("(input.pt_panel_minimiser)%2 == 0",
	conditionalPanel("(input.pt_import_panel_minimiser)%2 == 1",
  ...
  wellPanel("Bulk Composition",actionButton("bulk_comp_panel_minimiser", label = "..."),actionButton("bulk_import_panel_minimiser", label = "<-"),
  conditionalPanel("(input.bulk_comp_panel_minimiser)%2 == 0",
		conditionalPanel("(input.bulk_import_panel_minimiser)%2 == 1",
    ...
    uiOutput("trans"),
	conditionalPanel("input.bulk_def_file == false",
    uiOutput("maj"),
	...
	conditionalPanel("input.calculate_traces == true",
		selectInput("apply_trace_correction", "Apply saturation corrections",
			c("None","Zircon Saturation (Watson & Harrison 1983)","Monazite saturation (Montel 1993)","Zircon and Monazite saturation","Apatite saturation","Apatite & Monazite Saturation"),
			selected="None",
			selectize=TRUE),
		conditionalPanel("input.apply_trace_correction == \"Apatite & Monazite Saturation\"",
			fixedRow(
				column(4,
				...
				column(4, offset = 2,
					selectInput("apatite_saturation","Apatite saturation options",
						c("Harrison & Watson 1984","H&W with Bea et al. 1992","H&W with Pichavant et al. 1992","Wolf & London 1994"),
						selected="Harrison & Watson 1984",
						selectize=TRUE)))),
		conditionalPanel("input.apply_trace_correction == \"Apatite & Monazite Saturation\"",
			textInput("Xmz","Xmz (mole ratio LREE)",value = 0.83)),
		conditionalPanel("input.apply_trace_correction == \"Apatite saturation\"",
			selectInput("apatite_saturation","Apatite saturation options",
				c("Harrison & Watson 1984","H&W with Bea et al. 1992","H&W with Pichavant et al. 1992","Wolf & London 1994"),
				selected="Harrison & Watson 1984",
				selectize=TRUE)
  ...
  conditionalPanel("input.bulk_def_file == true",
...
tabPanel("Phase Manipulations",  
  #Phase Addition Panel
  wellPanel("Phase Addition",actionButton("phase_addition_panel_minimiser", label = "..."),actionButton("phase_addition_import_panel_minimiser", label = "<-"),
  conditionalPanel("(input.phase_addition_panel_minimiser)%2 == 0",
   conditionalPanel("(input.phase_addition_import_panel_minimiser)%2 == 1",
  ...
  checkboxInput("ph_add", "Perform Phase Addition?",value = FALSE),
    conditionalPanel("input.ph_add == true",
    textInput("n_ph_add_def","Number of addition definitions"),
      # Dynamic phase addition definition input boxes
      uiOutput("ph_add")
  ))),
  #Phase Extraction Panel
  wellPanel("Phase Extraction",actionButton("phase_extraction_panel_minimiser", label = "..."),actionButton("phase_extraction_import_panel_minimiser", label = "<-"),
  conditionalPanel("(input.phase_extraction_panel_minimiser)%2 == 0",
  conditionalPanel("(input.phase_extraction_import_panel_minimiser)%2 == 1",
  ...
  checkboxInput("ph_extr", "Perform Phase Extraction?",value = FALSE),
    conditionalPanel("input.ph_extr == true",
...
tabPanel("Modelling Options",
  #Modelling data Panel
  wellPanel("Modelling Data",actionButton("modelling_data_panel_minimiser", label = "..."),
   conditionalPanel("(input.modelling_data_panel_minimiser)%2 == 0",
...
#Additional optional parameters Panel
        wellPanel("Additional optional parameters",actionButton("additional_optional_parameters_panel_minimiser", label = "..."),
          conditionalPanel("(input.additional_optional_parameters_panel_minimiser)%2 == 0",
...
#Component Packet Panel
  wellPanel("Component Packet",actionButton("component_packet_panel_minimiser", label = "..."),
	conditionalPanel("(input.component_packet_panel_minimiser)%2 == 0",
		checkboxInput("component_packet", "Component Packet",value = FALSE),
		conditionalPanel("input.component_packet == true",
		tags$div(title="Enter components separated by commas (case-sensitive).\nIf the component does not exist in the thermodynamic data file then it will be created in Major elements input.",
  ...
  wellPanel("Extra Settings",actionButton("extra_settings_panel_minimiser", label = "..."),
   conditionalPanel("(input.extra_settings_panel_minimiser)%2 == 0",
