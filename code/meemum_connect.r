...
read.meemum <- function(meemum_in = meemum_in) {
  ...
  data_matrix_from_line <- function(line_text) {
    ...
    if (line_text == "Other Bulk Properties") {
      ...
      if (line_text == "Chemical Potentials") {
        text_altered[[2]] <- c("Bulk", text_altered[[2]])
      }
  ...
  npav <- function(grid_input, column1_val, column2_val) {
    ...
    contained <- TRUE
    if (!press <= max(grid_input[, 1])) {
      contained <- FALSE
    }
    if (!press >= min(grid_input[, 1])) {
      contained <- FALSE
    }
    if (!temp <= max(grid_input[, 2])) {
      contained <- FALSE
    }
    if (!temp >= min(grid_input[, 2])) {
      contained <- FALSE
    }
    if (contained) {
